...
def generate_launch_description():
    ...
    ekf_navsat = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([os.path.join(
            get_package_share_directory(
                package_name), 'launch', 'dual_ekf_navsat.launch.py'
        )])
    )
    ...
    keyboard = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([os.path.join(
            get_package_share_directory(
                package_name), 'launch', 'keyboard.launch.py'
    ...
    twist_mux_config = os.path.join(get_package_share_directory(
        package_name), 'config', 'twist_mux.yaml')
    twist_mux = Node(
        package='twist_mux',         # Package name for twist multiplexer
        executable='twist_mux',      # Executable name
        name='twist_mux',
        output='screen',
